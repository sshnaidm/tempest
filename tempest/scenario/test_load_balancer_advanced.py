...
class TestLoadBalancerBasic(manager.NetworkScenarioTest):
    ...
    def _create_lb_pool(self, lb_method='ROUND_ROBIN', protocol='HTTP'):
    ...
    def _create_load_balancer(self, lb_method, protocol, protocol_port):
        self._create_lb_pool(lb_method, protocol)
        self._create_members()
        self.vip = self._create_vip(protocol=protocol,
    ...
    def _check_tcp_load_balancing(self, port):
        """
        ...
        self._send_tcp_requests(self.vip_ip, self.servers.keys(), port)

    def _check_http_load_balancing(self):
        """
        1. Send NUM requests on the floating ip associated with the VIP
        2. Check that the requests are shared between the two servers
        """
        ...
        self._check_connection(self.vip_ip)
        self._send_http_requests(self.vip_ip, ["server1", "server2"])

    def _send_http_requests(self, vip_ip, servers):
    ...
    def _send_tcp_requests(self, vip_ip, servers, port, message=""):
        counters = dict.fromkeys(servers, 0)
        for i in range(self.num):
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            try:
                s.connect((vip_ip, port))
                s.send(message)
                server = s.recv(BUFFER_SIZE)
                counters[server] += 1
            except Exception:
                continue
            finally:
    ...
    def test_load_balancer_least_conn(self):
        ...
        self._create_servers(2)
        self._start_servers()
        self._create_load_balancer('LEAST_CONNECTIONS', 'HTTP', 80)
        self._check_load_balancing()
